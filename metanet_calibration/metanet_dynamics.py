...
def queue_dynamics(
    current: float, demand: float, flow_origin: float, T: float
) -> float:
    """Update queue length."""
...
def calculate_V(
    rho_per_lane, v_ctrl: float, a: float, rho_crit: float, v_free: float = 150.0
):
    ...
    if isinstance(rho_per_lane, np.ndarray):
        return np.minimum(
            v_free * np.exp(-((rho_per_lane / rho_crit) ** a) / a), v_ctrl
        )
...
def metanet_step(
    ...
    *,
    ...
    for i in range(num_segments):
        beta = _get_time_space_param(params["beta"], t, i)
        r = _get_time_space_param(params["r"], t, i)
        if i == 0:
            inflow = demand[t] if real_data else flow_origin_t
            outflow = density_t[i] * velocity_t[i]
            density_tp1[i] = density_dynamics(
                density_t[i], inflow, outflow, T, l, beta=beta, r=r
            )
        else:
...
def run_metanet_sim(
    ...
    flow[0] = np.array(
        [initial_density[i] * initial_velocity[i] for i in range(num_segments)],
        dtype=float,
    )
    ...
    total_travel_time = T * (
        sum([np.sum(density[:, i]) * l for i in range(num_segments)]) + np.sum(queue)
    )
    ...
    if plotting:
        ...
        density_per_lane = np.array(
            [density[:-1, i] / lanes[i] for i in range(num_segments)]
        ).T
